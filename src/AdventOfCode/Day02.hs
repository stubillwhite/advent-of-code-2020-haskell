...
-- >>> isValidPassword (PasswordEntry 1 2 'c' "abc")
-- True
-- >>> isValidPassword (PasswordEntry 2 3 'c' "abc")
-- False
isValidPassword :: PasswordEntry -> Bool
isValidPassword PasswordEntry{a,b,ch,password} = 
    a <= count && count <= b
  where
...
solutionOne input = 
    validPasswords
  where
    validPasswords = fmap (length . filter isValidPassword) (parseInput input)
